...
export function toAuctionFormValues(auction) {
  ...
  return {
    ...
    rewardMode: auction.reward_mode || (auction.auction_type === 'cash_amount' ? 'cash' : 'stock'),
    rewardValue: auction.reward_value ? String(Number(auction.reward_value).toFixed(2)) : '',
    cashPrize: auction.cash_prize || auction.each_winner_amount || auction.prize_amount ? String(Number(auction.cash_prize || auction.each_winner_amount || auction.prize_amount || 0).toFixed(2)) : '',
...
export function AuctionAdminForm({ initialValues, onSubmit, isSaving = false, submitLabel = 'Save Auction', products = [] }) {
  ...
  const isCashAuction = form.rewardMode === 'cash' || form.auctionType === 'cash_amount';
  const previewWinnerCount = Math.max(1, Number(form.winnerCount || 1));
  const previewCashPrize = Number(form.cashPrize || 0);
  ...
  function handleSubmit() {
    ...
    onSubmit({
      ...
      auctionType: isCashAuction ? 'cash_amount' : 'product',
      ...
      cashPrize: form.rewardMode === 'cash' ? Number(form.cashPrize || 0) : undefined,
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-white/10 bg-card p-5">
      <div className="grid gap-2 md:grid-cols-2">
        <button onClick={() => setForm((prev) => ({ ...prev, rewardMode: 'stock', auctionType: 'product' }))} className={`rounded-2xl border px-4 py-3 text-left ${form.rewardMode === 'stock' ? 'border-accent bg-accent/10 text-text' : 'border-white/10 bg-cardSoft text-muted'}`}>
          <p className="text-sm font-semibold">Stock Prize</p>
          <p className="mt-1 text-xs">Winners receive the configured stock quantity product reward.</p>
        </button>
        <button onClick={() => setForm((prev) => ({ ...prev, rewardMode: 'split', auctionType: 'product' }))} className={`rounded-2xl border px-4 py-3 text-left ${form.rewardMode === 'split' ? 'border-accent bg-accent/10 text-text' : 'border-white/10 bg-cardSoft text-muted'}`}>
          <p className="text-sm font-semibold">Split Reward</p>
          <p className="mt-1 text-xs">Winners split a configured reward value across the selected winner count.</p>
        </button>
      </div>

      <div className="grid gap-2">
        <button onClick={() => setForm((prev) => ({ ...prev, rewardMode: 'cash', auctionType: 'cash_amount' }))} className={`rounded-2xl border px-4 py-3 text-left ${form.rewardMode === 'cash' ? 'border-accent bg-accent/10 text-text' : 'border-white/10 bg-cardSoft text-muted'}`}>
          <p className="text-sm font-semibold">Cash Reward</p>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        {isCashAuction ? (
          <label className="space-y-2 text-sm text-muted">
            <span>Cash prize per winner</span>
            <input
              type="number"
              min="0.01"
              step="0.01"
              value={form.cashPrize}
              onChange={(e) => setForm((prev) => ({ ...prev, cashPrize: e.target.value }))}
              className="w-full rounded-2xl border border-white/10 bg-cardSoft px-3 py-2.5 text-sm text-text"
            />
          </label>
        ) : (
          <>
      ...
      {isCashAuction ? (
        <div className="rounded-2xl border border-white/10 bg-cardSoft p-3 text-xs text-muted">
          ...
          <p className="mt-1">Each winner receives: {formatAuctionMoney(previewCashPrize || 0)}</p>
          <p className="mt-1">Total payout liability: {formatAuctionMoney((previewCashPrize || 0) * previewWinnerCount)}</p>
        </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-cardSoft p-3 text-xs text-muted">
        ...
        {isCashAuction ? <p className="mt-1">Cash prize amount must be greater than $0.00 per winner and credits go to the withdrawal wallet.</p> : null}
