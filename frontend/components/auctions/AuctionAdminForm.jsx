...
export function toAuctionFormValues(auction) {
  ...
  return {
    ...
    startingPrice: String(Number(auction.starting_price || 0.5).toFixed(2)),
    minBidIncrement: String(Number(auction.min_bid_increment || 0.5).toFixed(2)),
...
export function AuctionAdminForm({ initialValues, onSubmit, isSaving = false, submitLabel = 'Save Auction' }) {
    ...
    <div className="space-y-4 rounded-3xl border border-white/10 bg-card p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <label className="space-y-2 text-sm text-muted">
          ...
          <input type="number" min="0.5" max="100" step="0.01" value={form.startingPrice} onChange={(e) => setForm((prev) => ({ ...prev, startingPrice: e.target.value }))} className="w-full rounded-2xl border border-white/10 bg-cardSoft px-3 py-2.5 text-sm text-text" />
        </label>
        <label className="space-y-2 text-sm text-muted">
          <span>Min increment</span>
          <input type="number" min="0.5" max="100" step="0.01" value={form.minBidIncrement} onChange={(e) => setForm((prev) => ({ ...prev, minBidIncrement: e.target.value }))} className="w-full rounded-2xl border border-white/10 bg-cardSoft px-3 py-2.5 text-sm text-text" />
        </label>
        <label className="space-y-2 text-sm text-muted">
          <span>Start at</span>
      ...
      <div className="rounded-2xl border border-white/10 bg-cardSoft p-3 text-xs text-muted">
        <p>Validated auction range: {formatAuctionMoney(form.startingPrice || 0.5)} to $100.00</p>
      ...
      <button
        onClick={() => onSubmit({
          ...
          startingPrice: Number(form.startingPrice || 0.5),
          minBidIncrement: Number(form.minBidIncrement || 0.5),
