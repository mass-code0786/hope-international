...
export default function AdminWithdrawalsPage() {
  ...
  const withdrawalsQuery = useQuery({
    queryKey: [...queryKeys.adminWithdrawals, search, status, page],
    queryFn: () => getAdminWithdrawals({ search, status, page, limit: 20 })
  });

  const reviewMutation = useMutation({
    mutationFn: ({ id, nextStatus, adminNote }) => reviewAdminWithdrawal(id, { status: nextStatus, adminNote }),
    onSuccess: async (result) => {
      toast.success(result.message || 'Withdrawal updated');
  ...
  return (
    <div className="space-y-5">
      <AdminSectionHeader title="Withdrawal Management" subtitle="Review and process withdrawal requests" />
      ...
      <DataTable
        ...
        rows={rows}
        empty={<EmptyState title="No withdrawals found" description="No withdrawal requests match your current filters." />}
      />
