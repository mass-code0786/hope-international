...
export default function AdminIncomePage() {
  ...
  const incomeQuery = useQuery({
    queryKey: [...queryKeys.adminIncome, search, source, page],
    ...
    <div className="space-y-5">
      ...
      <DataTable
        columns={[
          ...
          { key: 'status', title: 'Status', className: 'col-span-1', render: (row) => <StatusBadge status={row.metadata?.status || 'approved'} /> },
          { key: 'created_at', title: 'Date', className: 'col-span-2', render: (row) => dateTime(row.created_at) }
        ]}
        rows={rows}
        empty={<EmptyState title="No income records found" description="No income transactions match your filters." />}
      />
